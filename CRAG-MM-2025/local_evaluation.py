...
class CRAGEvaluator:
    ...
    def attempt_api_call(
        ...
        """
        使用 DeepSeek API 普通响应方式评估预测正确性。
        返回一个包含 .accuracy 和 .raw_content 字段的简单对象。
        """
        for attempt in range(max_retries):
            try:
                ...
                # 简单关键词判定正确性（你可以拓展为更复杂逻辑）
                if content == "correct":
                    accuracy = True
                elif content == "incorrect":
                    accuracy = False
                else:
                    print("⚠️ Unexpected evaluator output:", repr(content))
                    accuracy = False  # 保守默认
                return CRAGTurnEvaluationResult(accuracy=accuracy, raw_content=content)

            except Exception as e:
                print(f"[DeepSeek Retry {attempt + 1}] API call failed: {e}")
                if attempt == max_retries - 1:
                    print("[DeepSeek] All retries exhausted.")
        return None
    ...
    def evaluate_response(self, crag_turn_data: dict[str, any]) -> dict[str, any]:
        ...
        if not is_idk and not is_exact_match and self.eval_model_name:
            local_openai_client = OpenAI(
                api_key="sk-888b7df9c8dc415a8e2c9a60fcaf2e1e",
                base_url="https://api.deepseek.com"
            )

            messages = [
                {"role": "system", "content": "You are a strict evaluation assistant. "
                                              "Evaluate whether the Prediction correctly answers the Question compared to the Ground truth. "
    ...
    def generate_agent_responses(self, progress_callback: Callable[[int, int], None] = None) -> None:
        ...
        for batch_idx, batch in enumerate(
                tqdm.tqdm(self.batch_iterator, desc="Generating responses", disable=not self.show_progress)):
            ...
            # MAX_HISTORY_TURNS = 3  # 限制最多 3 轮历史
            for conversation_history in conversation_histories:
                ...
                # N = MAX_HISTORY_TURNS * 2  # 一轮 user + assistant 各 1 条
                message_histories.append(message_history)
                # message_histories.append(message_history[-N:])
                interaction_id_histories.append(interaction_id_history)
                # interaction_id_histories.append(interaction_id_history[-MAX_HISTORY_TURNS:])
            ...
            agent_responses = self.truncate_agent_responses(
                agent_responses)  # Truncase each response to the maximum allowed length (75 tokens)
    ...
    def evaluate_agent_responses(
        ...
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            ...
            for future_idx, future in tqdm.tqdm(enumerate(as_completed(futures)), total=len(futures),
                                                desc="Evaluating responses", disable=not self.show_progress):
...
def main() -> None:
    ...
    search_api_text_model_name = "/root/autodl-tmp/meta-comprehensive-rag-benchmark-starter-kit-main321/model/BAAI"
    search_api_image_model_name = "/root/autodl-tmp/meta-comprehensive-rag-benchmark-starter-kit-main321/model/models--openai--clip-vit-large-patch14-336/snapshots/ce19dc912ca5cd21c8a653c79e251e808ccabcd1"
